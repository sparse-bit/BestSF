...
double coo_kernel(d_coo &d_partition, d_array1D &d_x, d_array1D &d_y){

	double execution_time = 0.0;
	// compute y = A * x   
	cusp::multiply(d_partition, d_x, d_y);

	timer t_coo;
	for (int i = 0; i < NB_ITERATIONS; i++)
		cusp::multiply(d_partition, d_x, d_y);
	execution_time = t_coo.milliseconds_elapsed()/NB_ITERATIONS;

	return execution_time;
}

double csr_kernel(d_csr &d_partition, d_array1D &d_x, d_array1D &d_y){
	double execution_time = 0.0;
	// compute y = A * x   
	cusp::multiply(d_partition, d_x, d_y);
	...
	for (int i = 0; i < NB_ITERATIONS; i++)
		cusp::multiply(d_partition, d_x, d_y);
	execution_time = t_csr.milliseconds_elapsed() / NB_ITERATIONS;

	return execution_time;
}


double ell_kernel(d_ell &d_partition, d_array1D &d_x, d_array1D &d_y){
	...
	for (int i = 0; i < NB_ITERATIONS; i++)
		cusp::multiply(d_partition, d_x, d_y);
	execution_time = t_ell.milliseconds_elapsed() / NB_ITERATIONS;

	return execution_time;

}

double dia_kernel(d_dia &d_partition, d_array1D &d_x, d_array1D &d_y){
	...
	for (int i = 0; i < NB_ITERATIONS; i++)
		cusp::multiply(d_partition, d_x, d_y);
	execution_time = t_dia.milliseconds_elapsed() / NB_ITERATIONS;

	return execution_time;
}

double hyb_kernel(d_hyb &d_partition, d_array1D &d_x, d_array1D &d_y){
	...
	for (int i = 0; i < NB_ITERATIONS; i++)
		cusp::multiply(d_partition, d_x, d_y);
	execution_time = t_hyb.milliseconds_elapsed() / NB_ITERATIONS;
